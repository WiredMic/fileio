function DOMnode = writexml(varargin)
...
% Tibor Auer - Octave
    ...
    defaultXmlEngine  = 'Matlab';  % use matlab provided XMLUtils
    defaultCellItem   = true;  % allow cell arrays to use 'item' notation
    defaultRootOnly   = true;  % Input is root node only
    defaultStructItem = true;  % allow arrays of structs to use 'item' notation
    ...
    argParse.addRequired('filename',@ischar);
    argParse.addRequired('tree',@isstruct);
    argParse.addOptional('RootName','',@ischar);
    argParse.addParameter('ItemName',defaultItemName,@ischar);
    argParse.addParameter('XmlEngine',defaultXmlEngine,@(x) ischar(x) && ismember(x,{'Matlab' 'Xerces'}));
    argParse.addParameter('CellItem',defaultCellItem,@(x) islogical(x) || isnumeric(x));
    argParse.addParameter('RootOnly',defaultRootOnly,@(x) islogical(x) || isnumeric(x));
    argParse.addParameter('StructItem',defaultStructItem,@(x) islogical(x) || isnumeric(x));
    argParse.parse(varargin{:});
    DPref = argParse.Results;
    filename = DPref.filename;
    tree = DPref.tree;
    RootName = DPref.RootName;

    if isempty(RootName), RootName=inputname(2); end
    ...
    if(~DPref.RootOnly && isstruct(tree))  % if struct than deal with each field separatly
